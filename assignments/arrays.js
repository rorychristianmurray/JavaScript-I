...
let lastCar = 0;
console.log(`The make and model of the last car in the inventory is a ${inventory[49].car_make} ${inventory[49].car_model}`);
...
const carYears_1 = inventory.map(carYears_1 => carYears_1.car_year);


console.log(getYears(inventory, "car_year"));

console.log(carYears_1);
...
function getOldCars(objArray) {
    for (let i = 0; i <objArray.length; i++) {
        if (objArray[i].car_year < 2000) {
            oldCars.push(objArray[i]);
        }
    }

};

getOldCars(inventory);


console.log(`There are ${oldCars.length} cars older than 2000 in the lot`);
...
function getBMWAndAudi(objArray) {
    for (let i = 0; i < objArray.length; i++) {
        if (objArray[i].car_make === 'Audi' || objArray[i].car_make === 'BMW') {
            BMWAndAudi.push(objArray[i]);
        }
    }
}

getBMWAndAudi(inventory);
